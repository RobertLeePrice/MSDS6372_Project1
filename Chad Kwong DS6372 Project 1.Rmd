...
As we can see above, the VIF values for Infant Deaths and Under Five Deaths are extremely high. Based on this and the pairs result above, I have removed the variables infant.deaths and Thinness 10 to 19 years before recalculating the VIFs, I also remove the GDP variable because it seems to be correlated with Percentage Expendature.
...
After importing the olsrr package, a stepwise selection method was performed on the full cleaned model
...
```{r}
initialCustom.model = lm(Life.expectancy~Adult.Mortality+Income.composition.of.resources+Schooling+HIV.AIDS+Diphtheria+percentage.expenditure+BMI+Polio+Status+Measles, data = train)
plot(initialCustom.model)
summary(initialCustom.model)

v2.selection = lm(cleanedData$Life.expectancy~., data = cleanedData[,c(5, 8, 9, 10, 11, 12, 13, 15, 16, 21, 22)])
ols_step_both_p(v2.selection, prem = 0.05, pent = 0.05, details = FALSE)
...
ols_step_both_p(v3.selection, prem = 0.05, pent = 0.05, details = FALSE)
...
complexMLR.model = lm(Life.expectancy~Adult.Mortality+Income.composition.of.resources+Schooling+sqrt(HIV.AIDS)+Income.composition.of.resources*Schooling+sqrt(Income.composition.of.resources)*Schooling, data = train)
par(mfrow=c(2,2))
plot(complexMLR.model)

# Cooks D Analysis
cooksd <- cooks.distance(complexMLR.model)
# Plot the Cook's Distance using the traditional 4/n criterion
...
```{r}
#KNN Regression Model Using variables from selection3
# extracting x input and y labels 
...
test_y = test_y[order(test_y)]
pred_y = pred_y[order(pred_y)]
plot(x[1:100], test_y[1:100], col = "red", type = "l", lwd=2,
     main = "Life Expectancy test data prediction")
lines(x[1:100], pred_y[1:100], col = "blue", lwd=2)
#legend("topright",  legend = c("Actual Value", "Predicted Value"), 
       #fill = c("red", "blue"), col = 2:3,  adj = c(0, 0.6))
grid()
```
